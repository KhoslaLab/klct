# Setting Up R, RStudio, & klct

This is a brief, basic guide to setting up R, RStudio, and `klct` for
the first time. It assumes you’re using Windows.
...
We need to make sure the `tidyverse` R package is installed. We’ll also
use the `remotes` package to install `klct`, so that will need to be
installed as well.
